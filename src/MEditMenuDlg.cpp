...
void MAddMItemDlg::OnOK(HWND hwnd)
{
...
void MAddMItemDlg::OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	switch (id)
	{
	case IDOK:
...
BOOL MModifyMItemDlg::OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
...
void MModifyMItemDlg::OnOK(HWND hwnd)
{
	m_entry.szCaption = ::GetDlgItemTextW(hwnd, cmb1);
	if (!m_entry.szCaption.empty() && m_entry.szCaption[0] == L'"')
	{
...
void MModifyMItemDlg::OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	switch (id)
	{
	case IDOK:
...
BOOL MEditMenuDlg::OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
...
void MEditMenuDlg::OnAdd(HWND hwnd)
{
	MENU_ENTRY m_entry;
	m_entry.wDepth = 0;
...
BOOL MEditMenuDlg::GetEntry(HWND hwnd, MENU_ENTRY& entry, INT iItem)
{
...
BOOL MEditMenuDlg::SetEntry(HWND hwnd, MENU_ENTRY& entry, INT iItem)
{
	MStringW str, strIndent = LoadStringDx(IDS_INDENT);
...
void MEditMenuDlg::OnModify(HWND hwnd)
{
...
void MEditMenuDlg::OnDelete(HWND hwnd)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem >= 0)
	{
...
void MEditMenuDlg::OnUp(HWND hwnd)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem <= 0)
		return;
...
void MEditMenuDlg::OnDown(HWND hwnd)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem < 0)
		return;
...
void MEditMenuDlg::OnLeft(HWND hwnd)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem < 0)
		return;
...
void MEditMenuDlg::OnRight(HWND hwnd)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem < 0)
		return;
...
void MEditMenuDlg::OnOK(HWND hwnd)
{
	MENU_ENTRY entry;
...
void MEditMenuDlg::OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	switch (id)
	{
	case psh1:
...
void MEditMenuDlg::OnItemChanged(HWND hwnd)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
...
void MEditMenuDlg::OnInitMenuPopup(HWND hwnd, HMENU hMenu, UINT item, BOOL fSystemMenu)
{
	INT iItem = ListView_GetNextItem(m_hLst1, -1, LVNI_ALL | LVNI_SELECTED);
	BOOL bSelected = (iItem != -1);
...
LRESULT MEditMenuDlg::OnNotify(HWND hwnd, int idFrom, NMHDR *pnmhdr)
{
	if (idFrom == lst1)
	{

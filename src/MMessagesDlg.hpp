...
public:
    ...
    {
        ListView_GetItemText(m_hCtl1, iItem, 0, entry.MessageID, _countof(entry.MessageID));
        mstr_trim(entry.MessageID);

        ListView_GetItemText(m_hCtl1, iItem, 1, entry.MessageValue, _countof(entry.MessageValue));

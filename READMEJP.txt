...
[よくある質問]
    ...
    質問 3. 「リソースIDの一覧」から私のリソースIDが見つかりません。

        答え：最初に「ファイル」メニューから「IDの関連付け」を設定して下さい。
              リソースIDの名前を取得するのに resource.h を読み込む必要があります。
    ...
    質問 5. なぜVisual Studioでコンパイルすると、文字化けするの？
    ...
    質問 6. インストーラなしとポータブル版の違いは何？

        答え：ポータブル版はレジストリを使わず、iniファイルを使います。

    質問 7. 64ビットのファイルはサポートされていますか？

        答え：64ビットのWindowsでは「はい」です. しかしWoW64 は
              "C:\Program Files" や "C:\Windows\system32" からの読み込みを
...
[更新履歴]
    ...
    ver.5.2.9 (2020.02.01)
        ...
        バックアップ方法を修正。
    ver.5.3.0 (2020.02.06)
        いくつかのテキストボックスにES_AUTOHSCROLLを追加。
        上書き保存するときの論理エラーを修正。
    ver.5.3.1 (2020.02.23)
